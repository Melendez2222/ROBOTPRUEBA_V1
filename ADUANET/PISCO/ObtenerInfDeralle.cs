...
{
    internal class ObtenerInfDeralle
    {
        public async void ObtenerInfDetalle(IWebDriver driver, string digit)
        {
            ...
            for (int i = 0; i < detalleCells.Count; i++)
            {
                try
                {
                    ...
                    for (int attempt = 1; attempt <= 3; attempt++)
                    {
                        try
                        {
                            ...
                            elementFound = true;
                            Console.WriteLine("Elemento encontrado en el intento " + attempt);
                            break; 
                        }
                        catch (NoSuchElementException)
                        {
                ...
                {
                    writeLog.Log($"No se encontró el elemento en la celda de detalle: {ex.Message}.-{digit}");
                }
                catch (Exception ex)
                {
